...
class LesionTrainDataset(Dataset):
  def __init__(self, ISIC_image_path, image_path, notes_file):
    self.ISIC_img_path = ISIC_image_path
  ...
  def __getitem__(self, index):
    ...
    try:
      ...
      if 'ISIC' in img_name:
        img = Image.open(self.ISIC_img_path +img_name)
      else:
        img = Image.open(self.img_path + img_name)
    
    if 'ISIC' not in img_name: ### not ISIC image then crop the box
...
class LesionValDataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    if 'ISIC' not in img_name: ### if not ISIC image then crop the box
      bbox_x1 = self.train_data[index]['bbox'][0]
      bbox_y1 = self.train_data[index]['bbox'][1]
      bbox_x2 = bbox_x1 + self.train_data[index]['bbox'][2]
      bbox_y2 = bbox_y1 + self.train_data[index]['bbox'][3]
...
def train(args):
  train_loader = DataLoader(LesionTrainDataset(args.ISIC_train_image_path, args.train_image_path, args.train_notes_file), batch_size = args.batch_size, shuffle=True, num_workers=24)
  val_loader = DataLoader(LesionValDataset(args.ISIC_val_image_path, args.val_image_path, args.val_notes_file), batch_size = args.batch_size, shuffle=False, num_workers=8)
  # Define Model
...
if __name__ == "__main__":
  ...
  parser.add_argument("--ISIC_train_image_path", default = './Data/Images/ISIC/Train/', type = str, help = 'ISIC train images folder')
  parser.add_argument("--ISIC_val_image_path", default = './Data/Images/ISIC/Val/', type = str, help = 'ISIC val images folder') 
  parser.add_argument("--train_image_anno", default = './Data/Annotations/ISIC_augmentation/Only_Lesion/Train/', type = str, help = 'Train images annotations')
  parser.add_argument("--val_image_anno", default = './Data/Annotations/ISIC_augmentation/Only_Lesion/Val/', type = str, help = 'Val images annotations')
  parser.add_argument("--cls_num", default = 2, type = int, help = 'The number of classes for classification model')
  parser.add_argument("--save_path", default = '/Checkpoint/ISIC_Augmentation_Model/Only_Lesion', type = str, help = 'Checkpoints save path for ISIC augmentation model')
